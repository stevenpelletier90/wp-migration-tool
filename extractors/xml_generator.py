...
def generate_wordpress_xml_fixed(posts):
    ...
    for post in posts:
        ...
        # Content - wrap in Gutenberg Custom HTML block
        gutenberg_content = wrap_in_gutenberg_html_block(post['content'])
        xml_parts.append(f'<content:encoded><![CDATA[{gutenberg_content}]]></content:encoded>')
...
def wrap_in_gutenberg_html_block(content):
    """Wrap content in Gutenberg Custom HTML block"""
    if not content:
        return ""
    
    # Clean the content for CDATA
    clean_content = clean_content_for_cdata(content)
    
    # Wrap in Custom HTML block
    return f'<!-- wp:html -->\n{clean_content}\n<!-- /wp:html -->'
